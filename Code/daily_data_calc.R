...
daily_data<- fread("CropSyst/CropSyst_output/Washigton_watersheds/optimal/daily_result.dat", 
                   fill = TRUE, nrows = Inf) %>% 
  ...
  left_join(crops_grids %>% dplyr::select("crop","site","region","Irrigation","irrig_multiplier","yield_multiplier")) %>% 
  mutate(irrig1= irrig*irrig_multiplier) %>% 
...
path<- "D:/Coordinate_shutdown/Results of model run/NEW_Recalibrated/After_Recalibration/Data_relese/Washigton_watersheds/Scheduled_daily_data/"
...
write.csv(irrigation_demand1, "processed/CropSyst/Processed_seasonal_fm_daily.csv")
